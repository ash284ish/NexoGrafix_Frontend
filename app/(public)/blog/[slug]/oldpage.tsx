...
function CardShell({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    ...
    >
      <div className="text-base font-extrabold text-(--color-text-main)">{title}</div>
      ...
      <motion.section variants={pageWrap} initial="hidden" animate="show" className="relative overflow-hidden bg-white">
        <div className="relative mx-auto max-w-7xl px-6 py-14">
          <div className={cx("rounded-md border border-black/10 p-8", glassCard, "shadow-[0_18px_60px_rgba(15,23,42,0.08)]")}>
            <div className="text-sm font-extrabold tracking-[0.16em] text-(--color-text-main)">LOADING BLOG…</div>
            <div className="mt-3 text-sm font-semibold text-(--color-text-muted)">Fetching blog details content.</div>
          </div>
        </div>
      </motion.section>
    );
  ...
  if (loadErr) {
    return (
      <motion.section variants={pageWrap} initial="hidden" animate="show" className="relative overflow-hidden bg-white">
        <div className="relative mx-auto max-w-7xl px-6 py-14">
          <div className={cx("rounded-md border border-black/10 p-8", glassCard, "shadow-[0_18px_60px_rgba(15,23,42,0.08)]")}>
            <div className="text-sm font-extrabold tracking-[0.16em] text-(--color-text-main)">FAILED TO LOAD</div>
            <div className="mt-3 text-sm font-semibold text-(--color-text-muted)">
          ...
          <div className="min-w-0">
            ...
            >
              ...
              <div className={cx("px-6 pb-8 pt-7", glassSoft)}>
                <motion.div variants={itemUp} {...inViewOnce} className="space-y-5 text-[15px] font-semibold leading-relaxed text-(--color-text-muted)">
                ...
                {(post.content?.quote?.text || "").trim() ? (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="min-w-0">
                        <div className="text-[15px] font-extrabold leading-relaxed text-(--color-text-main)">
                  ...
                  <motion.div variants={itemUp} {...inViewOnce} className="mt-10 grid gap-7 lg:grid-cols-[360px_1fr] lg:items-start">
                    ...
                    <div className="space-y-4">
                      {(post.content?.sections ?? []).slice(0, 1).map((s, idx) => (
                        <div key={`s1-${idx}`}>
                          {s.heading ? (
                            <div className="text-[15px] font-extrabold text-(--color-text-main)">{s.heading}</div>
                          ) : null}
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        {(post.tags ?? []).slice(0, 6).map((t) => (
                          <span key={t} className={cx("rounded-md border border-black/10 px-3 py-1.5 text-xs font-extrabold text-(--color-text-main)", glassPill)}>
                            {t}
                          </span>
                        ))}
                      </div>
                ...
                <motion.div variants={itemUp} {...inViewOnce} className="mt-10 flex flex-col gap-4 border-t border-black/10 pt-6 sm:flex-row sm:items-center">
                  <div className="text-xs font-extrabold tracking-[0.18em] text-(--color-text-main)">TAGS</div>
          ...
          <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-6">
            <CardShell title="Search">
              <div className="flex items-center gap-3">
                ...
                <button className="inline-flex h-11 w-11 items-center justify-center rounded-md bg-(--color-brand) text-white shadow-sm transition hover:bg-(--color-brand-dark)">
                  <FiSearch />
                </button>
              </div>
            </CardShell>
                ...
                <AnimatePresence mode="popLayout">
                  {filteredRecent.map((r, i) => (
                    ...
                    >
                      <Link
                        href={`/blog/${r.slug}`}
                        className="block rounded-md border border-transparent px-2 py-2 text-sm font-semibold text-(--color-text-muted) transition hover:border-black/10 hover:bg-black/5 hover:text-(--color-text-main)"
                      >
