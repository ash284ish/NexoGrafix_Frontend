...
/** ---------- Dynamic content types (API) ---------- */
type PublishingDigitalJson = {
...
/** iconKey -> Icon component mapping for this page */
...
const heroInlineIconMap: Record<string, React.ComponentType<{ className?: string }>> = {
    layers: FiLayers,
...
export default function BookPublishingServicePage() {
    // Keep your current UI exact: we just hydrate these values from API.
    const [content, setContent] = useState<PublishingDigitalJson | null>(null);
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const json = (await res.json()) as PublishingDigitalJson;
                if (!cancelled) setContent(json);
            } catch {
                // UI same: if API fails, keep current static defaults (below).
    ...
    /** ---------- Defaults (your current hardcoded content) ---------- */
    const fallbackPageTitle = "Book Publishing";
    const fallbackPageSubtitle =
    ...
    const fallbackHeroImage =
        "https://images.unsplash.com/photo-1457694587812-e8bf29a43845?auto=format&fit=crop&w=2000&q=80";
    const fallbackPills = ["Digitization", "Editing", "eBook", "XML/HTML5", "Typesetting", "POD", "Interactive"];

    const pageTitle = content?.hero?.title ?? fallbackPageTitle;
    const pageSubtitle = content?.hero?.subtitle ?? fallbackPageSubtitle;
    const heroImage = content?.hero?.hero_image?.src ?? fallbackHeroImage;

    const pills = content?.hero?.pills?.length ? content.hero.pills : fallbackPills;

    const proofStats: StatItem[] = useMemo(() => {
        const stats = content?.trust_metrics?.stats;
        if (!stats?.length) {
            return [
                {
                    id: "projects",
        ...
        return stats.map((s) => {
            // trust metrics uses the same keys your ProofStatsSection already knows
            const icon =
                (defaultIcons as any)?.[s.iconKey] ??
    ...
    const services: ServiceItem[] = useMemo(() => {
        const slides = content?.services_carousel?.slides;
        if (!slides?.length) {
            ...
            return [
                {
                    ...
                    desc:
                        "We convert printed books, journals, and archives into high-quality digital formats that are searchable, clean, and production-ready.",
                    Icon: FiArchive,
                    image: {
                ...
                },
                {
                    ...
                    desc:
                        "We design professional interiors with clear hierarchy, precise layouts, and production-ready formatting for both print and digital.",
                    Icon: FiGrid,
                    image: {
        ...
        return slides.map((s) => {
            const Icon = serviceIconMap[s.iconKey] ?? FiFileText;
            return {
                id: s.id,
                title: s.title,
                desc: s.desc,
                Icon,
                image: { src: s.image?.src, alt: s.image?.alt ?? s.title },
                bullets: s.bullets ?? [],
            };
        });
    }, [content]);

    // Hero chips + badges from JSON (UI same, just data-driven)
    ...
    const heroBadges = useMemo(() => {
        ...
        return badges.map((b, idx) => {
            const Icon = heroInlineIconMap[b.iconKey] ?? FiCheckCircle;
            return (
    ...
    const servicesEyebrow = content?.services_carousel?.eyebrow ?? "SERVICES";
    const servicesHeading = content?.services_carousel?.heading ?? "Services & capabilities";
    const servicesSubheading =
        content?.services_carousel?.subheading ??
        "Explore our services through a guided carousel — each capability clearly highlights deliverables, quality standards, and turnaround timelines.";
    const autoRotateMs = content?.services_carousel?.auto_rotate_ms ?? 6500;
    const pauseOnHover = content?.services_carousel?.pause_on_hover ?? true;

    const showTabs = content?.services_carousel?.show_tabs ?? true;

    const trustEyebrow = content?.trust_metrics?.eyebrow ?? "TRUST METRICS";
    const trustHeading = content?.trust_metrics?.heading ?? "Publishing delivery you can count on";
    const trustSubheading = content?.trust_metrics?.subheading ?? "Results our publishing clients rely on";
    const trustColumns = content?.trust_metrics?.columns;
    const trustCentered = content?.trust_metrics?.centered ?? true;

    const safeColumns: 1 | 2 | 3 | 4 =
        trustColumns === 1 ||
            trustColumns === 2 ||
            trustColumns === 3 ||
            trustColumns === 4
            ? (trustColumns as 1 | 2 | 3 | 4)
            : 3;

    const contactEyebrow = content?.contact_cta?.eyebrow ?? "NEXT STEP";
    const contactHeading = content?.contact_cta?.heading ?? "Ready to discuss your publishing workflow?";
    const contactSubheading =
        content?.contact_cta?.subheading ??
        "Share your volume, formats (EPUB/XML/HTML5/PDF), and timelines, and we’ll recommend a clear delivery plan with defined quality checkpoints.";
    const contactPrimaryHref = content?.contact_cta?.primary_cta?.href ?? "/contact";
    const contactPrimaryLabel = content?.contact_cta?.primary_cta?.label ?? "Request a Quote";
    const contactBullets =
        content?.contact_cta?.bullets?.length
            ? content.contact_cta.bullets
            : [
    ...
    const contactNote = content?.contact_cta?.note_text ?? "Usually respond within 24 hours (business days)";

    const heroEyebrow = content?.hero?.eyebrow ?? "BOOK PUBLISHING";
    const heroPrimaryHref = content?.hero?.primary_cta?.href ?? "/contact";
            ...
            <div className="relative mx-auto max-w-7xl px-7 py-14 sm:px-8 sm:py-20">
                ...
                <div className="mx-auto mt-12 max-w-7xl">
                    <ServicesCarouselSection
                        ...
                        showTabs={showTabs} // <-- keep ON so "1..10 cursor/tab mover" works
                    />
                </div>
                ...
                <div className="mx-auto max-w-7xl">
                    <ContactCTASection
                        ...
                        primaryCta={{ href: contactPrimaryHref, label: contactPrimaryLabel }}
                        bullets={contactBullets}
                        noteText={contactNote}
                    />
                </div>
            </div>
        </motion.section>
