...
export default function SampleIndustriesPage() {
  ...
  const loadIndustries = async () => {
    try {
      setLoading(true);
      setErr(null);
      const res = await fetch(`${API_BASE}/api/v1/samples/industries`, {
        cache: "no-store",
      });
      ...
      setErr(e.message || "Failed to load industries");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = creating 
        ? `${API_BASE}/api/v1/samples/industries` 
        : `${API_BASE}/api/v1/samples/industries/${editing?.id}`;
      ...
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.detail || "Failed to save industry");
      }

      showToast("success", creating ? "Industry created successfully" : "Industry updated successfully");
      close();
      loadIndustries();
    } catch (e: any) {
  ...
  const handleDelete = async (i: IndustryRow) => {
    const ok = window.confirm(`Are you sure you want to delete industry "${i.name}"?`);
    if (!ok) return;

    try {
      const token = localStorage.getItem("access_token");
      const res = await fetch(`${API_BASE}/api/v1/samples/industries/${i.id}`, {
        method: "DELETE",
        headers: {
          "Authorization": `Bearer ${token}`,
        },
      ...
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.detail || "Failed to delete industry");
      }
  ...
  return (
    <div className="p-6">
      ...
      <SectionHeader
        title="Sample Industries"
        subtitle="Manage dynamic industries assigned to work samples."
        right={
      ...
      {err && (
        <div className="mt-5 rounded-md bg-red-50 p-4 ring-1 ring-red-200">
          <div className="flex gap-3">
            ...
            <div>
              <div className="text-sm font-semibold text-red-800">Error loading industries</div>
      ...
      <div className="mt-5 space-y-3">
        {loading ? (
          ...
          <div className="rounded-md bg-white p-6 text-center text-slate-500 shadow-sm ring-1 ring-slate-200/60">
            No industries found. Create an industry to get started.
          </div>
        ) : (
          industries.map((ind) => (
            <div key={ind.id} className="rounded-md bg-white p-5 shadow-sm ring-1 ring-slate-200/60">
              <div className="flex flex-wrap items-center justify-between gap-4">
                <div className="min-w-0">
                  <div className="text-lg font-semibold text-slate-900">{ind.name}</div>
                  <div className="mt-1 text-sm text-slate-500">Slug: <code className="rounded bg-slate-100 px-1 py-0.5 font-mono text-xs">{ind.slug}</code></div>
                </div>

                <div className="flex items-center gap-2">
                  <Button tone="slate" onClick={() => openEdit(ind)}>
                    <FiEdit2 /> Edit
                  </Button>
                  <Button tone="red" onClick={() => handleDelete(ind)}>
                    <FiTrash2 /> Delete
                  </Button>
                </div>
              </div>
      ...
      <Modal
        ...
        title={creating ? "Add Industry" : `Edit Industry: ${editing?.name}`}
        onClose={close}
        footer={
          <>
            ...
            <Button tone="orange" onClick={handleSave} disabled={submitting}>
              <FiSave /> {submitting ? "Saving..." : "Save Industry"}
            </Button>
          </>
        }
      >
        <div className="grid gap-4">
          <div className="grid gap-2">
            <label className="text-sm font-semibold text-slate-800">Industry Name</label>
            <input
              value={name}
