...
type PrivacySectionType = "META" | "SECTION" | "UNKNOWN";

type PrivacySection = {
  id: string;
  section_key: string;
  section_type: PrivacySectionType;
  title: string;
  subtitle: string;
  order_index: number;
  is_enabled: boolean;
...
function buildPrivacySections(json: any): PrivacySection[] {
  ...
  if (json?.meta) {
    ...
    sections.push({
      ...
      section_key: "meta",
      section_type: "META",
      title: toStr(m?.title || "Privacy Policy"),
      subtitle: `Last Updated: ${toStr(m?.lastUpdated || "—")}`,
      order_index: 1,
  ...
  if (Array.isArray(json?.sections)) {
    json.sections.forEach((s: any, idx: number) => {
      ...
      sections.push({
        ...
        title: toStr(s?.title || key),
        subtitle: `Blocks: ${blocksCount}`,
        order_index: 10 + idx,
        is_enabled: defaultEnabled,
...
export default function PrivacyPolicyAdminPage() {
  ...
  const endpoint = useMemo(
    () => joinApiUrl(API_BASE, "/api/v1/content/privacy-policy"),
    [API_BASE]
  );
  ...
  function persistToast(next: { tone: ToastTone; title: string; message?: string }) {
    try {
      sessionStorage.setItem("privacy_admin_toast", JSON.stringify(next));
    } catch {}
  }

  useEffect(() => {
    try {
      const raw = sessionStorage.getItem("privacy_admin_toast");
      if (!raw) return;
      sessionStorage.removeItem("privacy_admin_toast");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const built = buildPrivacySections(json);
        ...
        if (alive) setErr(e?.message || "Failed to load Privacy Policy");
      } finally {
  ...
  useEffect(() => {
    ...
    if (activeSection.section_key === "meta") {
      ...
      setMetaForm({
        title: toStr(m?.title),
        lastUpdated: toStr(m?.lastUpdated),
        lead: toStr(m?.lead),
      });
    }
    ...
    if (activeSection.section_type === "SECTION") {
      ...
      setSectionForm({
        key: toStr(s?.key || activeSection.section_key),
        title: toStr(s?.title || ""),
        blocksText: stringifyPretty(Array.isArray(s?.blocks) ? s.blocks : []),
      });
    }
  }, [activeSection]);

  async function saveActiveSection() {
    if (!rawPolicy || !activeSection) return;

    try {
      setSaving(true);
      setSaveErr(null);

      const updated = safeClone(rawPolicy);

      if (activeSection.section_key === "meta") {
        updated.meta = updated.meta || {};
        updated.meta.title = metaForm.title;
        updated.meta.lastUpdated = metaForm.lastUpdated;
      ...
      if (activeSection.section_type === "SECTION") {
        ...
        const key = toStr(sectionForm.key || activeSection.section_key);
        const title = toStr(sectionForm.title);

        if (!key) throw new Error("Section key is required");
        if (!title) throw new Error("Section title is required");

        const nextSection = {
          ...(typeof activeSection.raw === "object" && activeSection.raw ? activeSection.raw : {}),
          key,
          title,
          blocks: blocksParsed,
        };

        const idx = updated.sections.findIndex((x: any) => toStr(x?.key) === activeSection.section_key);
        if (idx === -1) {
          updated.sections.push(nextSection);
        } else {
          updated.sections[idx] = nextSection;
        }
      }

      const saveUrl = joinApiUrl(API_BASE, "/api/v1/content/privacy-policy");
...
/>
      ...
      <SectionHeader
        title="Privacy Policy Page Management"
        subtitle="Privacy policy content loaded from CMS JSON."
        right={
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-slate-500">
                    ...
                    <span className="rounded-full bg-slate-100 px-2.5 py-1 font-semibold">
                      Route: /api/v1/content/privacy-policy
                    </span>
                  </div>
                </div>
      ...
      <EditModal
        ...
        subtitle={
          activeSection ? "Update fields and save to /api/v1/content/privacy-policy" : ""
        }
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              ...
              <TextField
                ...
                onChange={(v) => setMetaForm((p) => ({ ...p, lastUpdated: v }))}
              />
            </div>
        ...
        {activeSection?.section_type === "SECTION" ? (
          <div className="space-y-6">
            ...
            <TextField
              ...
              onChange={(v) => setSectionForm((p) => ({ ...p, title: v }))}
            />
