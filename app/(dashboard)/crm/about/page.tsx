...
interface CMSData {
    meta?: { status?: string; updatedAt?: string; updatedBy?: string };
...
export default function AboutAdminPage() {
    ...
    const [rawAbout, setRawAbout] = useState<CMSData | null>(null);
    ...
    useEffect(() => {
        ...
        setSaveErr(null);
        setJsonErr(null);
        setEditOrder(asNumber((activeSection.raw as any)?.order, activeSection.order_index));
        try {
    ...
    const pageStatus = toStr((rawAbout as any)?.meta?.status || "");
    const updatedAt = toStr((rawAbout as any)?.meta?.updatedAt || "");
    const updatedBy = toStr((rawAbout as any)?.meta?.updatedBy || "");
