...
type RefundSectionType = "META" | "SECTION" | "SECTIONS_JSON" | "UNKNOWN";

type RefundAdminSection = {
  id: string;
  section_key: string;
  section_type: RefundSectionType;
  title: string;
  subtitle: string;
  order_index: number;
  is_enabled: boolean;
...
function buildRefundSections(json: any): RefundAdminSection[] {
  const sections: RefundAdminSection[] = [];
  ...
  if (json?.meta) {
    sections.push({
      ...
      subtitle: `Last Updated: ${toStr(json?.meta?.last_updated || "—")}`,
      order_index: 1,
  ...
  if (Array.isArray(json?.sections)) {
    json.sections.forEach((s: any, idx: number) => {
      const key = toStr(s?.key || `section_${idx + 1}`);
      const toc = toStr(s?.toc_title || "");
      const heading = toStr(s?.heading || "");
      const blocksCount = Array.isArray(s?.blocks) ? s.blocks.length : 0;

      sections.push({
        id: `section_${key}_${idx}`,
        section_key: `section:${key}`,
        section_type: "SECTION",
        title: heading || toc || key,
        subtitle: `TOC: ${toc || "—"} • Blocks: ${blocksCount}`,
        order_index: 10 + idx,
        is_enabled: defaultEnabled,
        meta: { badge: "SECTION", alignment: "left" },
        raw: s,
      });
    });
    ...
    sections.push({
      id: "sections_json",
      section_key: "sections_json",
      section_type: "SECTIONS_JSON",
      title: `Sections JSON (${json.sections.length})`,
      subtitle: "Bulk edit (advanced)",
      order_index: 999,
      is_enabled: defaultEnabled,
      meta: { badge: "JSON", alignment: "left" },
      raw: json.sections,
    });
  }
...
export default function RefundPolicyAdminPage() {
  ...
  const endpoint = useMemo(
    () => joinApiUrl(API_BASE, "/api/v1/content/refund-policy"),
    [API_BASE]
  );

  const [sections, setSections] = useState<RefundAdminSection[]>([]);
  const [rawRefund, setRawRefund] = useState<any>(null);
  ...
  function persistToast(next: { tone: ToastTone; title: string; message?: string }) {
    try {
      sessionStorage.setItem("refund_admin_toast", JSON.stringify(next));
    } catch {}
  }

  useEffect(() => {
    try {
      const raw = sessionStorage.getItem("refund_admin_toast");
      if (!raw) return;
      sessionStorage.removeItem("refund_admin_toast");
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!res.ok) throw new Error(`Failed to load Refund Policy (${res.status})`);

        const json = await res.json();
        const built = buildRefundSections(json);
        ...
        if (alive) setErr(e?.message || "Failed to load Refund Policy");
      } finally {
  ...
  const [sectionsJsonForm, setSectionsJsonForm] = useState({
    sectionsText: "",
  });
  ...
  useEffect(() => {
    ...
    if (activeSection.section_type === "SECTION") {
      const s = activeSection.raw || {};
      setSectionForm({
        key: toStr(s?.key),
        toc_title: toStr(s?.toc_title),
        heading: toStr(s?.heading),
        blocksText: stringifyPretty(s?.blocks || []),
      });
    }

    if (activeSection.section_key === "sections_json") {
      setSectionsJsonForm({
  ...
  async function saveActiveSection() {
    if (!rawRefund || !activeSection) return;

    try {
      setSaving(true);
      setSaveErr(null);

      const updated = safeClone(rawRefund);
      ...
      if (activeSection.section_type === "SECTION") {
        if (!Array.isArray(updated.sections)) updated.sections = [];

        const key = sectionForm.key.trim();
        if (!key) throw new Error("Section key is required");

        const blocksParsed = parseJsonSafe(sectionForm.blocksText);
        if (!Array.isArray(blocksParsed)) throw new Error("Blocks must be a JSON array");

        const idx = updated.sections.findIndex((x: any) => toStr(x?.key) === key);

        const nextSection = {
          ...(idx >= 0 ? updated.sections[idx] : {}),
          key,
          toc_title: sectionForm.toc_title,
          heading: sectionForm.heading,
          blocks: blocksParsed,
        };

        if (idx >= 0) updated.sections[idx] = nextSection;
        else updated.sections.push(nextSection);
      }

      if (activeSection.section_key === "sections_json") {
        const parsed = parseJsonSafe(sectionsJsonForm.sectionsText);
        if (!Array.isArray(parsed)) throw new Error("Sections must be a JSON array");
        updated.sections = parsed;
      }

      const saveUrl = joinApiUrl(API_BASE, "/api/v1/content/refund-policy");
...
/>
      ...
      <SectionHeader
        title="Refund Policy Page Management"
        subtitle="Refund Policy content loaded from CMS JSON."
        right={
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div className="min-w-0">
                  ...
                  <div className="mt-4 flex flex-wrap items-center gap-2 text-xs text-slate-500">
                    ...
                    <span className="rounded-full bg-slate-100 px-2.5 py-1 font-semibold">
                      Route: /api/v1/content/refund-policy
                    </span>
                  </div>
                </div>
      ...
      <EditModal
        ...
        subtitle={activeSection ? `Update fields and save to /api/v1/content/refund-policy` : ""}
      ...
      >
        ...
        {activeSection?.section_type === "SECTION" ? (
          <div className="space-y-6">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <TextField
                label="Section Key (immutable identifier)"
                value={sectionForm.key}
                onChange={(v) => setSectionForm((p) => ({ ...p, key: v }))}
              />
              <TextField label="Type" value="SECTION" onChange={() => null} disabled />
            </div>

            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <TextField
                label="TOC Title"
                value={sectionForm.toc_title}
                onChange={(v) => setSectionForm((p) => ({ ...p, toc_title: v }))}
              />
              <TextField
                label="Heading"
                value={sectionForm.heading}
                onChange={(v) => setSectionForm((p) => ({ ...p, heading: v }))}
              />
            </div>

            <TextAreaField
              label="Blocks (JSON array)"
              value={sectionForm.blocksText}
              onChange={(v) => setSectionForm((p) => ({ ...p, blocksText: v }))}
              rows={22}
            />
          </div>
        ) : null}

        {activeSection?.section_key === "sections_json" ? (
          <div className="space-y-5">
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
              <TextField label="Section Key" value="sections_json" onChange={() => null} disabled />
              <TextField label="Type" value="SECTIONS_JSON" onChange={() => null} disabled />
            </div>
            <TextAreaField
              label="Sections (JSON array) — bulk edit"
              value={sectionsJsonForm.sectionsText}
              onChange={(v) => setSectionsJsonForm({ sectionsText: v })}
              rows={26}
            />
          </div>
        ) : null}
