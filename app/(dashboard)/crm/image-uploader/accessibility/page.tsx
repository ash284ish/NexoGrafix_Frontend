...
import { FiEdit3, FiImage, FiAlertTriangle, FiUpload } from "react-icons/fi";
...
export default function AccessibilityImageAdminPage() {
    const API_BASE = process.env.NEXT_PUBLIC_API_BASE_URL || "";

    const endpoint = useMemo(
        () => buildApiUrl(API_BASE, "/api/v1/content/accessibilty-feature"),
        [API_BASE]
    );
    ...
    const uploadEndpoint = useMemo(
        () =>
            buildApiUrl(
                API_BASE,
                "/api/v1/content/accessibilty-feature/upload-image"
            ),
        [API_BASE]
    );
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                if (alive)
                    setErr(e?.message || "Failed to load accessibility content");
            } finally {
    ...
    return (
        <div className="p-6">
            ...
            <SectionHeader
                title="Accessibility Page – Images"
            ...
            {!loading && pageJson && (
                <div className="mt-6 space-y-4">
                    ...
                    <EditableSectionCard
                        ...
                        subtitle={`${pageJson?.services_carousel?.slides?.length || 0} slides`}
                        onEdit={() =>
                            setActiveSection({
