...
import { FiAlertTriangle, FiEdit3, FiImage, FiUpload } from "react-icons/fi";
...
type ImageSection =
    ...
    | { key: "services_carousel"; title: string };
...
export default function PublishingAdminPage() {
    ...
    const endpoint = useMemo(
        () => buildApiUrl(API_BASE, "/api/v1/content/publishing-digital"),
        [API_BASE]
    );
    ...
    const uploadEndpoint = useMemo(
        () =>
            buildApiUrl(
                API_BASE,
                "/api/v1/content/publishing-digital/upload-image"
            ),
        [API_BASE]
    );
    ...
    const [activeSection, setActiveSection] = useState<ImageSection | null>(null);
        ...
        if (activeSection.key === "services_carousel") {
            ...
            setCarouselImages(
                slides.map((s: any, idx: number) => ({
                    id: s?.id || String(idx),
                    title: toStr(s?.title || `Slide ${idx + 1}`),
    ...
    return (
        <div className="p-6">
            ...
            <SectionHeader
                title="Publishing Page – Images"
                subtitle="Manage image sources only (URL or local path)."
            />

            {err && (
                <div className="mt-4 rounded-md bg-white p-4 ring-1 ring-rose-200">
                    <FiAlertTriangle className="text-rose-600" /> {err}
                </div>
            )}
