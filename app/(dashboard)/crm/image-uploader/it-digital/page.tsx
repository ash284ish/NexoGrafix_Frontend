...
export default function ItServicesImageAdminPage() {
  const API_BASE = process.env.NEXT_PUBLIC_API_BASE_URL || "";
  const CONTENT_PATH = "/api/v1/content/it-developement";

  const endpoint = useMemo(() => buildApiUrl(API_BASE, CONTENT_PATH), [API_BASE]);
    ...
    try {
      ...
      if (uploadTarget.type === "carousel") {
        setCarouselImages((prev) => {
          const copy = [...prev];
          copy[uploadTarget.index] = { ...copy[uploadTarget.index], src: url };
          return copy;
        });
      }

      setToast({ open: true, tone: "success", title: "Image uploaded successfully" });
    } catch (e: any) {
      setToast({ open: true, tone: "error", title: "Upload failed", message: e?.message });
    } finally {
      setUploading(false);
  ...
  async function saveChanges() {
    ...
    try {
      ...
      setToast({ open: true, tone: "success", title: "Images updated successfully" });
      setActiveSection(null);
      setPageJson(updated);
    } catch (e: any) {
      setToast({ open: true, tone: "error", title: "Save failed", message: e?.message });
    } finally {
      setSaving(false);
    }
  }
      ...
      {!loading && pageJson && (
        <div className="mt-6 space-y-4">
          ...
          <EditableSectionCard
            ...
            onEdit={() =>
              setActiveSection({ key: "services_carousel", title: "Services Carousel" })
            }
          />
        </div>
      )}
      ...
      <EditModal
        ...
        footer={
          <div className="flex justify-end gap-2">
            <button className="rounded-md border px-4 py-2" onClick={() => setActiveSection(null)}>
              Cancel
            </button>
            <button
        ...
        {activeSection?.key === "hero" && (
          <div className="space-y-4">
            ...
            <textarea
              ...
              value={heroForm.src}
              onChange={(e) => setHeroForm((p) => ({ ...p, src: e.target.value }))}
            />

            <input
              className="w-full rounded border p-2 text-sm"
              value={heroForm.alt}
              onChange={(e) => setHeroForm((p) => ({ ...p, alt: e.target.value }))}
            />
          </div>
        )}
